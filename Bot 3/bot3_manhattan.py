...
def is_unblocked(grid, row, col, avoid_adjacent_fire=False):
    ...
    if avoid_adjacent_fire:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = row + dx, col + dy
            if is_valid(nx, ny, n) and grid[nx][ny] == 2:
...
def time_lapse_fn(grid, q, n, frames, src, dest, fire_init):
    ...
    log_data = {
        ...
        'steps': 0,
        'result': '',
        'final_frame': f'/Users/drcrocs22/Developer/Rutgers Projects/Intro To AI/Project 1 - Grid Bot/final_frames/{run_id}.png'
    }
    ...
    def plan_path():
        ...
        found_dest = False
        cell_details, found_dest = bot_planning(closed_list, cell_details, open_list, bot_pos, dest, grid, found_dest, n, avoid_adjacent_cells=True)
        if found_dest:
            return track_path(cell_details, dest, bot_pos, n)
        else:
            ...
            heapq.heappush(open_list, (0.0, (i, j)))
            found_dest = False
            cell_details, found_dest = bot_planning(closed_list, cell_details, open_list, bot_pos, dest, grid, found_dest, n, avoid_adjacent_cells=False)
            if found_dest:
                return track_path(cell_details, dest, bot_pos, n)
            else:
                return None
    ...
    save_final_frame(frames[-1], filename=f'/Users/drcrocs22/Developer/Rutgers Projects/Intro To AI/Project 1 - Grid Bot/final_frames/{run_id}.png')
...
def bot_planning(closed_list, cell_details, open_list, src, dest, grid, found_dest, n, avoid_adjacent_cells):
    while len(open_list) > 0:
        ...
        for dir in directions:
            ...
            if is_valid(new_i, new_j, n):
                ...
                if not closed_list[new_i][new_j] and is_unblocked(grid, new_i, new_j, avoid_adjacent_cells) and not is_fire(grid, new_i, new_j):
...
def log_results(log_data, filename='/Users/drcrocs22/Developer/Rutgers Projects/Intro To AI/Project 1 - Grid Bot/simulation_results.csv'):
...
# Main Execution for Bot 3
n = 40
q = 1.0  # Adjusted fire spread probability for testing
grid = grid_init(n)
frames = []
